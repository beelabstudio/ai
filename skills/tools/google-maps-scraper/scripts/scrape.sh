...
# BLS fix: macOS ships bash 3.2 by default (`env bash` resolves to it unless a newer bash is
# installed and earlier in PATH), and bash <4.4 treats an EMPTY array as unset under `set -u`,
# so a bare double-quoted "AUTH[@]" expansion aborts with "unbound variable" on every run without
# SCRAPER_API_KEY set (the default — this kit needs no key for localhost). Every use below is
# written as "${AUTH[@]+"${AUTH[@]}"}" instead — the standard bash-3.2-safe idiom that only
# expands AUTH's elements when AUTH is actually set/non-empty.
