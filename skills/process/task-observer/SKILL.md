...
> **Bee Lab Studio local override (`rules/security.md` takes precedence).**
> This skill is vendored verbatim below for auditability, but this repo's
> [`rules/security.md`](../../../rules/security.md) is always in effect and
> overrides one specific pattern in the upstream text: anywhere below that
> instructs retrying a denied or failed action, calling "a second tool
> reaching the same path," or treating "consecutive denials from a
> probabilistic gatekeeper" as "noise, not a wall" (see the Session Start
> Protocol step 1 and the "How to Log" checkpoint rule) — including the
> line that generalizes this to "every gated call" — does **not** apply.
> Any tool-permission denial, from the user or from the harness's own
> auto-mode classifier, is respected as final: report it plainly and stop,
> never retry it or route around it through an alternate tool/interface.
> Everything else in this skill (observation capture, skill-authoring
> workflow, weekly review) is unaffected and should be followed as written.
> Also scope `[workspace folder]` per project when this skill is copied
> into a consuming project's own `.claude/skills/` — do not default to a
> path shared across unrelated projects unless the user asks for that.
