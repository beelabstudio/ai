...
source: https://github.com/sergebulaev/instagram-skills
---

# Instagram Marketing Skills

Vendored from the upstream `instagram-skills` project (see `source` above); re-check
upstream before assuming behaviour has changed. Only skill content and the Python
`lib/` helpers were vendored — upstream's own plugin/marketplace scaffolding
(`.claude-plugin/`, `.codex-marketplace/`, release tooling, etc.) was dropped since
it doesn't apply to how skills are consumed in this repo. `PUBLORA_API_KEY`,
`APIFY_TOKEN`, and `PIXFARO_TOKEN` are optional third-party API keys (see
`.env.example`) — never commit real values; follow `rules/security.md` for secret
handling.
